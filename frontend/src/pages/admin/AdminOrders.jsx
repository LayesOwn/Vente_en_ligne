                    ...
                    <div className="px-4 py-4 bg-beige-50">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex-1 flex justify-end items-end gap-2">
                          {(() => {
                            const orderNum = `DASHA-${String(order.id).padStart(4, "0")}`;
                            const msgFn = STATUS_MESSAGES[order.status];
                            let text = msgFn ? msgFn(orderNum) : null;
                            if (text && order.public_token) {
                              const trackUrl = `${window.location.origin}/confirmation/${order.public_token}`;
                              text += `\n\nSuivez votre commande ici : ${trackUrl}`;
                            }
                            const waLink = text ? buildWaLink(order.phone, text) : null;
                            ...
                            return (
                              <a
                                ...
                                title="Envoyer une notification de suivi au client"
                              >
                                <WhatsAppIcon />
                                Prévenir le client
                              </a>
